...
export default class TaskclusterProxy {
  ...
  async link(task) {
    ...
    if (process.env.DEBUG.includes(this.featureName)) {
      // Do not capture stdout as it looks like it might container the tasckluster
      // credentials.  taskcluster-proxy should be revised
      let stream = await this.container.attach({stream: true, stdout: false, stderr: true});
