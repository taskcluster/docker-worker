...
TaskclusterProxy.prototype = {
  ...
  */
  ...
  link: function* (task) {
    ...
    var assumedScope = 'queue:assume:worker-id:' + task.config.workerGroup +
                       '/' + task.config.workerId;

    var cmd = [
        '--client-id=' + task.config.conf.get('taskcluster:clientId'),
        '--access-token=' + task.config.conf.get('taskcluster:accessToken'),
        task.status.taskId,
        assumedScope
    ];
