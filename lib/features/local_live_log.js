...
export default class TaskclusterLogs {
  constructor() {
    ...
    */
    ...
    this.artifactPath = undefined; // will be overriden in link()
  }

  static getArtifactPathFromTask(task) {
    return task && task.payload && task.payload.log ?
        task.payload.log : ARTIFACT_PATH;
  }
  ...
  async link(task) {
    ...
    this.artifactPath = TaskclusterLogs.getArtifactPathFromTask(task.task);

    let directory = path.parse(this.artifactPath).dir;
