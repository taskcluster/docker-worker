...
export default class DockerSave {
  async killed (task) {
    ...
    debug(task.task.payload.features.dockerSave);
    debug(task.task.payload.features);
    let artifactName = task.task.payload.features.dockerSave.artifactPath || ARTIFACT_NAME;
    debug(artifactName);
