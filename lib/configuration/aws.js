...
/**
@return Number of minutes left in the billing cycle.
*/
function* billingCycleMinutesRemaining() {
  // We use os.uptime to attempt to figure out our billing cycle which may or
  // may not be the perfect number here...
  var uptimeMinutes = os.uptime() / 60;
  return Math.ceil(BILLING_CYCLE_MINUTES - uptimeMinutes % 60);
}


module.exports.billingCycleMinutesRemaining = billingCycleMinutesRemaining;
...
function* configure (baseUrl) {
  ...
  // Number of minutes left in cycle when we can trigger a shutdown.
  config.shutdownMinutesStart = 12;

  // Number of minutes left where we do _not_ shutdown (wait for another cycle).
  config.shutdownMinutesStop = 2;
