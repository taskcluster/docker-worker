...
// TODO consider renaming.  Not really listening for tasks but rather managing
// tasks in the queue and claiming work
/**
...
export default class TaskListener extends EventEmitter {
  ...
  //no need for task listener to subscribe to this, just have the shutdown manager
  //call terminate on this.
  listenForShutdowns() {
    // If node will be shutdown, stop consuming events.
    if (this.runtime.shutdownManager) {
      this.runtime.shutdownManager.once(
        'shutdownManager:nodeTermination', async () => {
          await this.pause();
  ...
  async getTasks() {
    ...
    if (availabileCapacity <= 0) { scheduleTaskPoll(); return;}
    ...
    for(let queue of queues) {
      ...
      while(claims.length < availabileCapacity) {
        ...
        await Promise.all(newClaims.map(async (claim) => { await this.runTask(claim) }));
  ...
  scheduleTaskPoll() {
    this.pollTimeoutId = setTimeout(async () => {
      try {
        ...
        debug(e);
        this.runtime.log('[alert-operator] task error',
          {
            taskId: task.taskId,
            runId: task.runId,
            message: e.toString(),
  ...
  async connect() {
    ...
    this.cancelListener = this.listenForCancelEvents();
