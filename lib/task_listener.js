...
export default class TaskListener extends EventEmitter {
  ...
  async getTasks() {
    ...
    for(let queue of queues) {
      ...
      while(claims.length < availabileCapacity) {
        ...
        let tasks = await taskQueue.getTasksFromQueue(queue, tasksNeeded);
  ...
  async claimTask(task) {
    try {
      let claim = await this.runtime.stats.timeGen(
        'tasks.time.claim',
      ...
      if ((500 <= e.statusCode && e.statusCode < 600) || e.statusCode === 401) {
        throw e;
      }
      // 4xx errors should result in the task being removed from the queue.  Task
      // either is not pending or was claimed by another worker
      if (400 <= e.statusCode && e.statusCode < 500) {
        await this.taskQueue.deleteTaskFromQueue(task);
      }

      this.runtime.log('claim task error', {
        taskId: task.taskId,
        runId: task.runId,
        err: e,
        stack: e.stack
