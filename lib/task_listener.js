...
function TaskListener(runtime) {
  ...
  if (this.runtime.shutdownManager) {
    this.runtime.shutdownManager.once(
      'shutdownManager:nodeTermination', co(function* () {
        yield this.pause();
...
TaskListener.prototype = {
  ...
  */
  ...
  connect: function* () {
    ...
    // Maintain a state for the task listener so tasks are not claimed when
    // the node needs to finish work before shutting down,  Can't close the channel
    // because tasks can still report to the queue.
    ...
    var result = yield channel.consume(listener._queueName, co(function* (msg) {
    ...
    // Need to set this directly when interacting directly with the channel rather
    // than calling resume() on the listener.  Necessary for pausing a listener
    // using .pause()
    this.listener._consumerTag = result.consumerTag;
  },

  close: function* () {
    // close listeners before closing the pulse connection
    yield [
      this.listener.close(),
      this.cancelListener.close(),
    ];
    return yield this.pulseConnection.close();
  },
