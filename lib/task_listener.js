...
export default class TaskListener extends EventEmitter {
  ...
  async getTasks() {
    ...
    claims.forEach(this.coalesceClaim.bind(this)); // NOTE: ignores errors
  }
  ...
  */
  ...
  async coalesceClaim(claim) {
    try {
      ...
      // see if this task is coalescible
      if (!task.payload.coalescer) {
        return await this.runTaskset([claim]);
      }

      // TODO: get message routes, look for one with a prefix matching
      // task.payload.coalescer.routePrefix.  More than one is an error;
      // zero matches just falls back to runTaskset([claim]).

      // TODO: for the matching route, append the suffix to
      // task.payload.coalescer.url and fetch a list of taskIds.

      // TODO: call queue.claimTask for all taskIds in the list (just returning
      // the existing claim for claim.status.taskId); ignore failed calls, and
      // collect the results of the successful calls into a list.  Call
      // runTaskset with this list.
    } catch (e) {
