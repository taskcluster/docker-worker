...
var VideoDeviceManager = require('./video_device_manager');
var AudioDeviceManager = require('./audio_device_manager');
...
export default class TaskListener extends EventEmitter {
  constructor(runtime) {
    ...
    this.videoDeviceManager - new VideoDeviceManager();
  ...
  */
  async runTask(claim) {
    try {
      ...
      if (task.payload.devices) {
        runningState.devices = {};
        debug('Task payload specifies devices');
        for (let device in task.payload.devices) {
        ...
        options.devices = runningState.devices;
      }
