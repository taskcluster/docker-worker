...
function ShutdownManager(host, config) {
  ...
  // Recommended by AWS to query every 5 seconds.  Spot termination window is 2 minutes
  // so at the very least should have 1m55s to cleanly shutdown.
  this.spotTerminationTimeout = setTimeout(
    this.spotNodeTerminated.bind(this), config.shutdown.nodeTerminationPoll || 5000
  );
}
...
ShutdownManager.prototype = {
  ...
  */
  ...
  spotNodeTerminated: co(function* () {
    clearTimeout(this.spotTerminationTimeout);
    var terminated = yield this.host.getSpotTerminationTime();

    if (terminated) {
      this.emit('shutdownManager:nodeTermination', terminated);
    }

    this.spotTerminationTimeout = setTimeout(
      this.spotNodeTerminated.bind(this), 5000
    );
  })
};
