...
export default class ShutdownManager extends EventEmitter {
  ...
  async shutdown() {
    ...
    this.stats.increment('workerShutdown', {value: Date.now()});
    // TODO: make sure points are flushed before shutting down?
  ...
  scheduleTerminationPoll() {
    return async () => {
      ...
      if (terminated) {
        ...
        this.stats.increment('workerSpotTermination', Date.now());
      }
