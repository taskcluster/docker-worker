...
GarbageCollector.prototype = {
  ...
  markImage: function(image) {
    ...
    if (!(parseRepositoryTag(imageName).tag)) {
      imageName += ':latest';
    }

    var expiration = new Date(new Date().getTime() + this.imageExpiration);
  ...
  removeUnusedImages: function* () {
    ...
    for (var image in this.markedImages) {
      ...
      if (this.markedImages[image] > new Date()){
  ...
  sweep: function () {
    ...
    co(function* () {
      ...
      var exceedsThreshold = yield exceedsDiskspaceThreshold('/',
                               this.diskspaceThreshold,
                               (this.capacity - this.taskListener.pending));

      if (this.capacity - this.taskListener.pending > 0) {
        if (exceedsThreshold) {
          this.log('diskspace threshold reached');
          this.emit('gc:warning', {message: 'Diskspace threshold reached.'});
          yield this.removeUnusedImages();
        } else {
          console.log(exceedsThreshold);
          this.log('not reached');
          this.emit('gc:info', {message: 'Diskspace threshold not reached.'});
        }
      }
