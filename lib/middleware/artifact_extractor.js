...
ArtifactExtractor.prototype = {
  ...
  putArtifact: function* (taskHandler, name, path) {
    ...
    try {
      ...
      // Log the error...
      debug('Error extracting artifact', {
        path: path, name: name, taskId: taskHandler.status.taskId
      });
    ...
    tarExtract.on('entry', co(function* (header, stream, callback) {
      ...
      if (res.error) {
        debug('Error uploading artifact', {
          path: path, name: entryName, taskId: taskHandler.status.taskId
        });
  ...
  extractResult: function (result, taskHandler) {
    ...
    return coPromise(function* () {
      ...
      result.result.artifacts = yield artifactResults;
      return result;
    });
  }
};
