...
/** Extract artifacts */
ArtifactExtractor.prototype.extractResult = function(result, taskHandler) {
  ...
  for(artifact in artifacts) {
    urlRequest.artifacts[artifact] = {
      contentType: mime.lookup(artifact)
    };
  }

  var gotArtifactPutUrls =
    queue.requestArtifactUrls(taskHandler.status.taskId, urlRequest);
  ...
  return gotArtifactPutUrls.then(function(result) {
    var artifactUrls = result.artifacts;
