...
Task.prototype = {
  ...
  */
  ...
  pullDockerImage: function() {
    ...
    var credentials = dockerImage.credentials(this.runtime.registry);
    if (credentials) {
      // Validate scopes on the image if we have credentials for it...
      if (!scopeMatch(payload.image, this.task.scopes)) {
        throw new Error(
          'Insufficient scopes to pull : "' + payload.image + '" try adding ' +
          payload.image + ' to the .scopes array.'
        );
      }
  ...
  run: function* () {
    ...
    var runtimeTimeoutId = setTimeout(function() {
      ...
      this.stream.write(this.fmtLog(
        TASK_TIMEOUT_ERROR, this.task.payload.maxRunTime
      ));
