  ...
  */
  dockerConfig: function() {
    ...
    return {
      ...
      create: {
        ...
        OpenStdin: false,
        StdinOnce: false,
        Env: taskEnvToDockerEnv(config.env || [])
      }
    }
  },
  ...
  run: function* () {
    ...
    var dockerProc = this.dockerProcess = new DockerProc(
      this.config.docker, this.dockerConfig()
    );
    ...
    // Run any middleware which generates the log files.
    //var logs = yield middleware.run('declareLogs', {}, task, dockerProcess);
  }
};
