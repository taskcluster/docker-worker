...
export default class Task {
  ...
  */
  ...
  /**
   * Resolves a run and reports the completion to the proper exchange.
   *
   * If a task was aborted because of a malformed payload or worker shutdown,
   * the task will be reported as an exception.
   *
   * No Action will be taken for a canceled task because it has already been canceled
   * by an external entity. Canceling the task run within the worker allows resources
   * to be cleaned up.
   */
  async completeRun(success) {
    ...
    if (this.taskException) {
      ...
      this.taskState = success ? 'completed' : 'failed';
      if (success) {
        await this.runtime.stats.timeGen(
        'tasks.time.completed',
        queue.reportCompleted(this.status.taskId, this.runId, {success: success})
        );
      }
      else {
        await this.runtime.stats.timeGen(
  ...
  */
  async run() {
    ...
    if (this.isCanceled() || this.isAborted()) return false;
    ...
    if (this.isCanceled() || this.isAborted()) {
      await this.abortRun(this.taskState)
      return false;
    }
    try {
    ...
    try {
      ...
      await this.abortRun('docker_configuration', error);
      return false;
    }
