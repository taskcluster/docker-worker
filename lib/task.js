...
export class Reclaimer {
  ...
  async reclaimTask() {
    ...
    try {
      ...
      if (this.claim != this.primaryClaim) {
        this.stop();
        return;
      }
  ...
  */
  ...
  async resolveSuperseded(primaryTaskId, primaryRunId, addArtifacts, reason) {
    ...
    try {
      await Promise.all(this.claims.map(async function(c){
        ...
        if (addArtifacts) {
          ...
          await uploadToS3(c, contentJson, "public/superseded-by.json", expiration, {
    ...
    if (addArtifacts) {
      ...
      await uploadToS3(c, contentJson, "public/supersedes.json", expiration, {
        'content-type': 'application/json',
        'content-length': contentJson.length,
      });
    }
