...
import { IMAGE_ERROR } from './docker_image';
...
export class Task {
  ...
  */
  async run() {
    ...
    let imageId;
    try {
      imageId = await this.runtime.imageManager.ensureImage(this.task.payload.image, this.stream)
      /*
      image = await pullDockerImage(
        this.runtime,
      ...
      */
      ...
      return await this.abortRun(
        'pull_failed',
        this.fmtErrorLog(IMAGE_ERROR, this.runtime.imageManager.getReadableImageName(this.task.payload.image), e)
      );
    }
