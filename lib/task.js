...
Task.prototype = {
  ...
  */
  ...
  run: function* () {
    ...
    var taskclusterProxy = yield*
      stats.timeGen('tasks.time.proxy', this.createTaskclusterProxy());
    ...
    yield stats.timeGen(
      'tasks.time.middleware.started', middleware.run('start', this)
    );
    ...
    yield stats.timeGen(
      ...
      middleware.run('declareLogs', logs, this)
    );
    ...
    yield stats.timeGen(
      ...
      middleware.run('extractResult', result, this)
    );
    ...
    yield stats.timeGen(
      ...
      middleware.run('finalizeLogs', result, this)
    );

    // Post the result json so it is available when the task is marked complete.
    var resultReq = stats.timeGen(
      'tasks.time.result_put',
      yield put(claim.resultPutUrl, result)
    );
    ...
    yield stats.timeGen(
      ...
      queue.reportTaskCompleted(this.status.taskId, completeOptions)
    );

    stats.increment('tasks.completed');
  }
};
