 ...
 *     maxMessagesPerRequest: // max Number of messages to retrieve in a single request
 *   }
 * }
 *
 */
export default class TaskQueue {
  constructor(config) {
    ...
    assert(config.taskQueue.maxMessagesPerRequest, 'Max number of messages to retrieve at a time from the queue is required');
    this.queues = null;
    ...
    this.maxMessagesPerRequest = config.taskQueue.maxMessagesPerRequest;
  ...
  async getTasksFromQueue(queue, numberOfTasks) {
    let maxMessages = Math.min(numberOfTasks, this.maxMessagesPerRequest);
    let tasks = [];
